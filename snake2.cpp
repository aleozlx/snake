...
GLuint VAO, VBO;
GLint u_offset, u_color, u_scale, u_shape_type, u_inner_radius, u_texture, u_use_texture, u_aspect_ratio;
...
void initializeGame() {
    ...
    if (level >= 2) {
        ...
        std::cout << "Food physics initialized for Level " << level << " (GRAVITY mode)" << std::endl;
    }
}
...
void drawCircle(float x, float y, float diameter, float r, float g, float b) {
    ...
    if (debugCount < 5) {
        std::cout << "Circle: diameter=" << diameter << " aspectRatio=" << aspectRatio << " (no correction for manual debug)" << std::endl;
        debugCount++;
    }
...
void drawPerfectCircle(float centerX, float centerY, float radius, float r, float g, float b) {
    ...
    glUniform2f(u_scale, diameter, diameter); // Force square: same dimension for both X and Y
    glUniform3f(u_color, r, g, b);
    glUniform1i(u_shape_type, 1); // Circle shape
...
bool initializeGyroscope() {
    ...
    if (SDL_InitSubSystem(SDL_INIT_SENSOR) < 0) {
        std::cout << "Failed to initialize SDL sensor subsystem: " << SDL_GetError() << std::endl;
        return false;
    }
    ...
    if (numSensors == 0) {
        ...
        std::cout << "1. Enable 'Generic Gamepad Configuration' in Steam" << std::endl;
        std::cout << "2. Set Gyro to 'As Joystick' in controller settings" << std::endl;
        std::cout << "3. Try disabling Steam Input for this game" << std::endl;
        std::cout << "4. Check if gyro works in Steam's controller test" << std::endl;
        return false;
    }
    ...
    for (int i = 0; i < numSensors; i++) {
        ...
        switch (sensorType) {
            case SDL_SENSOR_GYRO: hasGyroscope = true; break;
            case SDL_SENSOR_ACCEL: hasAccelerometer = true; break;
            case SDL_SENSOR_GYRO_L: hasLeftGyro = true; break;
            case SDL_SENSOR_GYRO_R: hasRightGyro = true; break;
            case SDL_SENSOR_ACCEL_L: hasLeftAccel = true; break;
            case SDL_SENSOR_ACCEL_R: hasRightAccel = true; break;
        }
        
        std::cout << "Sensor " << i << ":" << std::endl;
        std::cout << "  Name: " << (sensorName ? sensorName : "Unknown") << std::endl;
        std::cout << "  Type: " << sensorType << " (";
        
        switch (sensorType) {
            case SDL_SENSOR_INVALID: std::cout << "INVALID"; break;
            case SDL_SENSOR_UNKNOWN: std::cout << "UNKNOWN"; break;
            case SDL_SENSOR_ACCEL: std::cout << "ACCELEROMETER"; break;
            case SDL_SENSOR_GYRO: std::cout << "GYROSCOPE"; break;
            case SDL_SENSOR_ACCEL_L: std::cout << "LEFT_ACCELEROMETER"; break;
            case SDL_SENSOR_GYRO_L: std::cout << "LEFT_GYROSCOPE"; break;
            case SDL_SENSOR_ACCEL_R: std::cout << "RIGHT_ACCELEROMETER"; break;
            case SDL_SENSOR_GYRO_R: std::cout << "RIGHT_GYROSCOPE"; break;
            default: std::cout << "TYPE_" << sensorType; break;
        }
        std::cout << ")" << std::endl;
        ...
        if (sensorType == SDL_SENSOR_GYRO || 
            ...
            if (gyroSensor) {
                ...
                std::cout << "🌀 Gyroscope initialized: " << (sensorName ? sensorName : "Unknown Gyro") << std::endl;
    ...
    std::cout << std::endl << "No gyroscope found, checking for accelerometer as fallback..." << std::endl;
    for (int i = 0; i < numSensors; i++) {
        SDL_SensorType sensorType = SDL_SensorGetDeviceType(i);
        
        if (sensorType == SDL_SENSOR_ACCEL || 
            sensorType == SDL_SENSOR_ACCEL_L || 
            sensorType == SDL_SENSOR_ACCEL_R) {
            
            std::cout << "Found accelerometer, attempting to use for tilt detection..." << std::endl;
            gyroSensor = SDL_SensorOpen(i);
            if (gyroSensor) {
                gyroSupported = true;
                std::cout << "✅ Using accelerometer for Level 2 GRAVITY mode" << std::endl;
                std::cout << "Note: This uses tilt instead of rotation for food movement" << std::endl;
                std::cout << "=============================" << std::endl;
                return true;
            }
        }
...
void updateFoodPhysics(float deltaTime) {
    if (!gyroSupported || level < 2) return;
    ...
    if (SDL_SensorGetData(gyroSensor, sensorData, 3) == 0) {
        ...
        if (sensorType == SDL_SENSOR_GYRO || 
            ...
            // Use tilt detection - when device tilts, accelerometer shows gravity direction
            // Normalize and scale the accelerometer data
        ...
        if (bounced || (currentTime - lastDebugTime > 2.0f && (abs(foodVelocityX) > 0.1f || abs(foodVelocityY) > 0.1f))) {
            const char* sensorTypeName = "UNKNOWN";
            if (sensorType == SDL_SENSOR_GYRO) sensorTypeName = "GYRO";
            else if (sensorType == SDL_SENSOR_ACCEL) sensorTypeName = "ACCEL";
            
            std::cout << "🌀 " << sensorTypeName << ": X=" << forceX << " Y=" << forceY 
                      << " | Food vel: X=" << foodVelocityX << " Y=" << foodVelocityY << std::endl;
...
void render() {
    ...
    if (level == 0) {
        ...
        drawText("LVL 2", levelTextX, levelTextY, textSize, 0.8f, 0.8f, 0.8f); // Gray text
    }
    ...
    if (level == 0) {
        ...
        if (gyroSupported) {
            drawText("GRAVITY", levelTextX, levelTextY - textSize * 1.2f, textSize * 0.7f, 0.0f, 1.0f, 1.0f); // Cyan text for active gravity
        } else {
            drawText("GRAVITY", levelTextX, levelTextY - textSize * 1.2f, textSize * 0.7f, 0.5f, 0.5f, 0.5f); // Gray text for disabled gravity
        }
    }
    ...
    if (level >= 2) {
        ...
        if (gyroSupported) {
            ...
            float velocityMagnitude = sqrt(foodVelocityX * foodVelocityX + foodVelocityY * foodVelocityY);
            if (velocityMagnitude > 0.1f) {
                drawText("GYRO ACTIVE", gyroTextX, gyroTextY, textSize * 0.5f, 0.0f, 1.0f, 0.0f); // Green when active
            } else {
                drawText("GYRO READY", gyroTextX, gyroTextY, textSize * 0.5f, 0.0f, 0.8f, 1.0f); // Light blue when ready
            }
            ...
            if (velocityMagnitude > 0.05f) {
                ...
                drawText(velText, gyroTextX, barTextY, textSize * 0.4f, 1.0f, 1.0f, 0.0f); // Yellow velocity text
            }
        } else {
            drawText("NO GYRO", gyroTextX, gyroTextY, textSize * 0.5f, 1.0f, 0.0f, 0.0f); // Red when not available
        }
    }
    ...
    if (lastButtonPressed >= 0 && usingGamepadInput) {
        ...
        float sensorTextX = buttonTextX + textSize * 4.5f; // To the right of "GAMEPAD"
        ...
        if (hasGyroscope || hasLeftGyro || hasRightGyro) {
            drawText("GYRO", currentX, sensorTextY, sensorFontSize, 0.0f, 1.0f, 1.0f); // Cyan for detected
            currentX += textSize * 2.5f;
        } else {
            drawText("GYRO", currentX, sensorTextY, sensorFontSize, 1.0f, 0.0f, 0.0f); // Red for not detected
            currentX += textSize * 2.5f;
        }
        
        // Accelerometer sensors
        if (hasAccelerometer || hasLeftAccel || hasRightAccel) {
            drawText("ACCEL", currentX, sensorTextY, sensorFontSize, 0.0f, 1.0f, 1.0f); // Cyan for detected
            currentX += textSize * 3.0f;
        } else {
            drawText("ACCEL", currentX, sensorTextY, sensorFontSize, 1.0f, 0.0f, 0.0f); // Red for not detected
            currentX += textSize * 3.0f;
        }
        
        // Left sensors (if detected separately)
        if (hasLeftGyro || hasLeftAccel) {
            drawText("L", currentX, sensorTextY, sensorFontSize, 0.0f, 1.0f, 1.0f); // Cyan for detected
            currentX += textSize * 1.0f;
        }
        
        // Right sensors (if detected separately)
        if (hasRightGyro || hasRightAccel) {
            drawText("R", currentX, sensorTextY, sensorFontSize, 0.0f, 1.0f, 1.0f); // Cyan for detected
        }
    }
...
void changeLevel(int newLevel) {
    ...
    if (level >= 2 && gyroSupported) {
        ...
        foodVelocityY = 0.0f;
        lastGyroUpdateTime = 0.0f;
        std::cout << "🌀 GRAVITY mode activated! Tilt device to move the food!" << std::endl;
    } else if (level >= 2 && !gyroSupported) {
        std::cout << "⚠️  Level 2 GRAVITY mode requires gyroscope - not available" << std::endl;
